...
class BlobTracker:
    def __init__(self, minTravelBetweenKeyframes: float, detectorDescriptorAndComparer: FeatureDetectorDescriptorAndComparer, fTestMaxVal: float, matchingScoreLowerLimit: float, numKfsToGoBackWhenFindingCorrespondences: int, logger=None):
        self.totalTravel = 0
        self.T_prev = None
        self.minTravelBetweenKeyframes = minTravelBetweenKeyframes
    ...
    def handleNewFrame(self, image, T, pose_noise_sigmas, frameName):
        ...
        # If a sufficient amount of travel has occurred, handle keyframe
        if self.totalTravel > self.travelAtLatestKeyframe + self.minTravelBetweenKeyframes:
